...
try {
    ...
    $duration1 = round(microtime(true) - $start, 4);

    // --------------------------------------------
    // 2. Optimize
    //    - Add Fulltext Indexes on All Filtered Columns
    //    - Replace LIKE '%keyword%' With MATCH ... AGAINST
    //    - Remove Unused LEFT JOINs if not needed
    // --------------------------------------------
    $keyword = 'キャビンアテンダント';
    $start = microtime(true);
    $sql2 = "
        ...
        WHERE (
            MATCH(Jobs.name, description, detail, business_skill, knowledge, location, activity, salary_statistic_group, salary_range_remarks, restriction, remarks)
              AGAINST ('$keyword' IN NATURAL LANGUAGE MODE)
            OR MATCH(JobCategories.name) AGAINST ('$keyword' IN NATURAL LANGUAGE MODE)
            OR MATCH(JobTypes.name) AGAINST ('$keyword' IN NATURAL LANGUAGE MODE)
            OR MATCH(Personalities.name) AGAINST ('$keyword' IN NATURAL LANGUAGE MODE)
        )
    ...
    $duration2 = round(microtime(true) - $start, 4);

    // ------------------------------
    // Output Results
    // ------------------------------
    echo "Original Query: " . count($rows1) . " rows in {$duration1}s\n";
    echo "Optimize Query: " . count($rows2) . " rows in {$duration2}s\n";
